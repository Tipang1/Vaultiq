...
class Vault:
    ...
    def search_entries(self, query, fields=None):
        ...
        if 'service' in fields:
            results.update([e for e in self.entries if q in e.service.lower()])
        if 'username' in fields:
            results.update([e for e in self.entries if q in e.username.lower()])
        if 'notes' in fields:
            results.update([e for e in self.entries if q in e.notes.lower()])
